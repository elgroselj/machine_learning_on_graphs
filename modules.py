...
class BlockDiagonalLinear(torch.nn.Module):
    ...
    def enforce_block_diagonal(self):
        ...
        for i in range(self.num_blocks):
            ...
            mask[start0:end0, start1:end1] = torch.ones(end0-start0, end1-start1)
...
class MySAGEConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.project:
            if in_channels[0] <= 0:
                raise ValueError(f"'{self.__class__.__name__}' does not "
                                 ...
                                 f"`project=True`")
            self.lin = BlockDiagonalLinear(in_channels[0], in_channels[0], num_blocks, bias=True)
        ...
        self.lin_l = BlockDiagonalLinear(aggr_out_channels, out_channels, num_blocks,  bias=bias)
        if self.root_weight:
            self.lin_r = BlockDiagonalLinear(in_channels[1], out_channels, num_blocks, bias=False)
...
class MyHeteroGraphSAGE(torch.nn.Module):
    def __init__(
        ...
        for _ in range(num_layers):
            conv = HeteroConv(
                {
                    edge_type: MySAGEConv((channels, channels), channels, aggr=aggr, num_blocks=num_blocks)
